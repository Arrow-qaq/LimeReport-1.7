...
namespace LimeReport
{
    ...
    ScriptEditor::ScriptEditor(QWidget* parent)
        : QWidget(parent), ui(new Ui::ScriptEditor), m_reportEngine(0),
          m_page(0), m_tabIndention(4)
    {
        ui->setupUi(this);
        ...
        ui->textEdit->setTabStopWidth(
            ui->textEdit->fontMetrics().boundingRect("0").width() *
            m_tabIndention);
#else
        ui->textEdit->setTabStopDistance(
            ui->textEdit->fontMetrics().boundingRect("0").width() *
            m_tabIndention);
#endif
        ...
        connect(
            ui->textEdit, SIGNAL(textChanged()), this, SIGNAL(textChanged()));
    }
    ...
    void ScriptEditor::setReportEngine(
        ReportEnginePrivateInterface* reportEngine)
    {
    ...
    void ScriptEditor::setPageBand(BandDesignIntf* band)
    {
        if (band && ui->twData->model() && !band->datasourceName().isEmpty())
        {
            QModelIndexList nodes = ui->twData->model()->match(
                ui->twData->model()->index(0, 0), Qt::DisplayRole,
...
#if QT_VERSION < QT_VERSION_CHECK(5, 12, 3)
            ui->textEdit->setTabStopWidth(
                ui->textEdit->fontMetrics().boundingRect("W").width() *
                charCount);
#else
            ui->textEdit->setTabStopDistance(
                ui->textEdit->fontMetrics().boundingRect("W").width() *
    ...
    void ScriptEditor::on_twData_doubleClicked(const QModelIndex& index)
    {
        ...
        LimeReport::DataNode* node =
            static_cast<LimeReport::DataNode*>(index.internalPointer());
        if (node->type() == LimeReport::DataNode::Field)
        {
            ui->textEdit->insertPlainText(QString("$D{%1.%2}")
    ...
    void ScriptEditor::slotOnCurrentChanged(
        const QModelIndex& to, const QModelIndex&)
    {
        LimeReport::ScriptEngineNode* node =
    ...
    QString ReportStructureCompleater::pathFromIndex(
    ...
    void ReportStructureCompleater::addAdditionalDatawords(
        CompleterModel* model, DataSourceManager* dataManager)
    {
        ...
        // #ifdef USE_QJSENGINE
        //     ScriptEngineManager& se =
        //     LimeReport::ScriptEngineManager::instance(); QJSValue
        //     globalObject = se.scriptEngine()->globalObject();
        ...
        //                     for (int i = 0; i<
        //                     it.value().toQObject()->metaObject()->methodCount();++i){
        //                         if
        //                         (it.value().toQObject()->metaObject()->method(i).methodType()
        //                         == QMetaMethod::Method){
        //                             CompleterItem* methodNode = new
        //                             CompleterItem; QMetaMethod m =
        //                             it.value().toQObject()->metaObject()->method(i);
        //                             QString methodSignature = m.name() + "(";
        //                             bool isFirst = true;
        //                             for (int j = 0; j <  m.parameterCount();
        //                             ++j){
        //                                     methodSignature += (isFirst ? ""
        //                                     : ",") + m.parameterTypes()[j]+"
        //                                     "+m.parameterNames()[j];
    ...
    void ReportStructureCompleater::updateCompleaterModel(
        ReportEnginePrivateInterface* report)
    {
        if (report)
        {
            m_newModel.clear();
            ...
            for (int i = 0; i < report->pageCount(); ++i)
            {
                ...
                foreach (
                    BaseDesignIntf* item, page->pageItem()->childBaseItems())
                {
                    addChildItem(
                        item, itemNode->text(), m_newModel.invisibleRootItem());
                }
...
#else
                    result.append(
                        QString::fromLatin1(mo->method(i).signature()));
#endif
                }
            }
            mo = mo->superClass();
    ...
    QStringList ReportStructureCompleater::extractProperties(
        BaseDesignIntf* item)
    {
        QStringList result;
        if (!item) return result;
    ...
    void ReportStructureCompleater::addChildItem(
        BaseDesignIntf* item, const QString& pageName, CompleterItem* parent)
    {
        if (!item) return;
        ...
        //        QSharedPointer<CacheItem> cacheItem =
        //        QSharedPointer<CacheItem>(new CacheItem);
